...
const finalGroupingBasedOnStatuses = (
    allPossiableFileCommitsTestData: EachFilePerCommitInfo_Summary[],
) => {
    const filteringMap = new Map();

    allPossiableFileCommitsTestData.forEach(
        (eachFileData: {
            fileName: string;
            fileRemainingInfo: {
                fileStatus: string;
            ...
            if (
                ...
                if (
                    ...
                    // If the File is renamed/modified Initialy Now it is Renamed -->Keeping it as Renamed...Updating the Commit Message & Other Info
                    filteringMap.has(eachFileData.fileName) &&
...
export const getSummaryObject = (formattedData: FormattedData) => {
    const fileAndRemObj: EachFilePerCommitInfo_Summary[] =
        formattedData.listOfFormattedCommitData.flatMap((acc) => {
            const getAllFiles = (eachCommit: FormattedData["listOfFormattedCommitData"][0]) => {
                const result = eachCommit.eachCommitFiles.map(
