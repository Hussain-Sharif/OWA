import { AllSettledRepoOutput_Summary, CommitsObject_Summary, EachRepoCommitInfo_Summary, EachRepoInfo, EachUserInfo, FinalIndividualCommitResponse, FinalUserCommitsData_Summary, FormattedAllSettledRepoOutput_Summary, GithubBadResponseType, GithubGoodResponseType } from "../libs/types";
...
const getEachRepoCommitInfo=async(eachRepoPerUser:EachRepoInfo)=>{
    ...
    const gettingAllCommitsInfo: GithubGoodResponseType | GithubBadResponseType = await getGithubRepoInfo(
        ...
        const newformattedSummaryData:CommitsObject_Summary=getSummaryObject(finalGoodResponse.formattedData);
...
const commitLaunchpad=async(eachUserInfo:EachUserInfo)=>{
    ...
    const formattedRepoOutputs:FormattedAllSettledRepoOutput_Summary = allRepoOutputs.map((result, idx) =>
